<template>
  <div class="column-flights">
    <div v-for="(item, index) in itemCard.segments" :key="index" class="flights-results-itinerary">
      <div class="column">
        ...
        <p>
          {{ getCity(item, 'arrival', 'at') | filterTime }} {{ getIATACode(item, 'arrival') }} ({{
            getCity(item, 'arrival', 'iataCode')
          }})
        </p>
      </div>
      <div v-if="index < itemCard.segments.length - 1" class="line">
        <div class="transfer">
          ...
          <span class="display-none-desktop">{{
            getLayoverTime(itemCard, index, 'onlyTime')
          }}</span>
        </div>
      </div>
    </div>
