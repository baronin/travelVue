<template>
  <div v-show="!hide" class="flight-info-wrap">
    ...
    >
      ...
      <div class="row">
        ...
        <itinerary-results :item-card="itemCard"></itinerary-results>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      arrivalsAndDeparture: ['Outbound', 'Return'],
      hide: false,
      toFrom: '',
  ...
  computed: {
    ...mapGetters('global', ['durationTime']),
  },
  ...
  methods: {
    ...
    getTimeArrival(item, direction, directionArr) {
      ...
      if (item.segments.length === 1) {
        ...
        let departureDay = moment(item.segments[0].flightSegment.departure.at).format('D')
        let arrivalDay = moment(
          item.segments[item.segments.length - 1].flightSegment.departure.at
        ).format('D')
        if (departureDay !== arrivalDay) {
          return moment(data).format('ddd, MMM D')
        } else if (directionArr) {
    ...
    getTotalTime(value) {
      ...
      if (value.segments.length > 1) {
        let departureTime = moment
          .utc(moment(value.segments[0].flightSegment.departure.at))
          .format('x')
        let arrivalTime = moment
          .utc(moment(value.segments[value.segments.length - 1].flightSegment.arrival.at))
          .format('x')
