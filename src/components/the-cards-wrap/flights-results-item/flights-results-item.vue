<template>
  <div>
    <div
      v-for="(segments, index) in flightCardsData.offerItems[0].services"
      :key="index"
      class="flights-results-item"
    >
      <div class="column">
...
export default {
  ...
  methods: {
    ...
    toArrivedTime(value) {
      return moment(value.segments[value.segments.length - 1].flightSegment.arrival.at).format(
        'H:mm'
      )
    },
    toTotalTime(value) {
    ...
    getLayoverTime(value, index) {
      if (value.segments.length > 1) {
        for (let i = index; i < value.segments.length - 1; i++) {
          ...
          let departureTime = moment(
            moment(value.segments[i + 1].flightSegment.departure.at)
          ).format('x')
