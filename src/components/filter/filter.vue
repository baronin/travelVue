    ...
    <div class="filter-wrapper">
        ...
        <div class="content-wrap">
          <label @click="displayFloatingFilterButton">
            <input
              ...
              value="withLuggage"
            />
            <span class="checkbox-custom" @click="displayFloatingFilterButton"></span>
      ...
      <div class="filter-item-wrap">
        ...
        <div class="content-wrap">
          <div class="range-value">
            ...
            <span v-if="value < maxDuration" class="chosen-value">{{ value | toHours }}</span>
            <span v-else class="chosen-value">any</span>
          </div>
      ...
      <div class="filter-item-wrap">
        ...
        <template v-for="stop in dataForFilter.stops">
          <div :key="stop.code" class="content-wrap">
            <label @click="displayFloatingFilterButton">
              <input
      ...
      <div class="filter-item-wrap">
        <div class="title-button-wrap">
          ...
          <button type="button" class="btn bg-none clear-btn" @click="clearPrice">Clear</button>
        </div>
        <div class="content-wrap">
          <div class="range-value">
      ...
      <div class="filter-item-wrap">
        ...
        <template v-for="(airline, index) in dataForFilter.carrierCode">
          <div v-if="index < variable" :key="airline.value" class="content-wrap">
            <label @click="displayFloatingFilterButton">
              <input
      ...
      <div class="filter-item-wrap">
        ...
        <template v-for="departItem in dataForFilter.departure">
          <div :key="departItem.value" class="content-wrap">
            <label @click="displayFloatingFilterButton">
              <input
        ...
        <template v-for="item in dataForFilter.arrival">
          <div :key="item.value" class="content-wrap">
            <label @click="displayFloatingFilterButton">
              <input
      ...
      <div class="filter-footer">
        ...
        <div class="filter-button">
          <the-button :modifier="[{ 'is-primary': true }]" @button-click="setFilteredData"
            >Filter
          </the-button>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('api', ['dataFromApi', 'filteredArrayForCounting']),
  },
  ...
  methods: {
    ...
    getAirlines() {
      let arr = this.dataFromApi.data
        .map((item) =>
          item.offerItems[0].services.map((service) =>
            service.segments.map((segment) => segment.flightSegment.carrierCode)
          )
        )
        .flat(2)
      let dictionariesKeys = Object.keys(this.dataFromApi.dictionaries.carriers)
      this.dataForFilter.carrierCode = dictionariesKeys
        .filter((item) => {
    ...
    getIataDeparture() {
      ...
      for (let i = 0; i < this.dataFromApi.data.length; i++) {
        departArr.push(
          this.dataFromApi.data[i].offerItems[0].services[0].segments[0].flightSegment.departure
            .iataCode
        )
      }
    ...
    getIataArrival() {
      ...
      for (let i = 0; i < this.dataFromApi.data.length; i++) {
        ...
        arrivalArr.push(
          this.dataFromApi.data[i].offerItems[0].services[0].segments[targetSegment].flightSegment
            .arrival.iataCode
        )
      }
      arrivalArr.sort()
        ...
        for (let service in this.dataFromApi.data[i].offerItems[0].services) {
          if (this.dataFromApi.data[i].offerItems[0].services[service].segments.length > 1) {
            let departure = '',
              arrival = '',
              targetSegment =
                this.dataFromApi.data[i].offerItems[0].services[service].segments.length
            for (let segment in this.dataFromApi.data[i].offerItems[0].services[service].segments) {
              if (segment === 0) {
                departure =
                  this.dataFromApi.data[i].offerItems[0].services[service].segments[segment]
            ...
            let dep =
              this.dataFromApi.data[i].offerItems[0].services[service].segments[0].flightSegment
                .departure.at
            let arrival =
              this.dataFromApi.data[i].offerItems[0].services[service].segments[0].flightSegment
                .arrival.at
    ...
    getPriceValues() {
      ...
      unsortedArr.sort()
      let minPrice = unsortedArr[0],
        maxPrice = unsortedArr[0]
