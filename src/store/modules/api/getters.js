  ...
  filteredArrayForCounting(state) {
    ...
    return state.dataFromApi.data
        .filter(offer => {
          ...
          return state.filters.total !== undefined
              && jsonPrice <= state.filters.total[1]
              && jsonPrice >= state.filters.total[0];
        })
        .filter(offer => {
          for (let i = 0; i < offer.offerItems[0].services.length; i++) {
            const segments = offer.offerItems[0].services[i].segments;
            for (let j = 0; j < segments.length; j++) {
              const indexOfStops = state.filteredArray.stops.findIndex(
        ...
        .filter(offer => () => {
          ...
          })(offer.offerItems[0].services) <= state.filteredArray.duration;
        })
        .filter(offer => () => {
          return ((offer, data, code, codeName) => {
            ...
            for (let i = 0; i < services.length; i++) {
              ...
              for (let j = 0; j < segments.length; j++) {
                let indexOfCodeName = data.findIndex(
                    item => item[code] ===
                        segments[j].flightSegment[codeName] &&
                        item.isChecked);
                if ((indexOfCodeName > -1)) {
                  return true;
                }
            ...
            return false;
          })(offer, state.filteredArray.carrierCode, 'code', 'carrierCode');
        })
        .filter(offer => () => {
          return filterByAirports(offer, state.filteredArray.departure,
              'iataCode');
        })
        .filter(offer => () => {
          return filterByAirports(offer, state.filteredArray.arrival,
              'iataCode');
        });
  },
};
