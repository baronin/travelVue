...
<script>
  ...
  export default {
    ...
    computed: mapState(['dataForApi', 'breadCrampsActive', 'invalidFormsOnPassengerInfoPage']),
    methods: {
      getToPage(item, index) {
        this.isActive = index;
        if(item !== 'review' ){
          this.$router.push({ name: item, query: this.dataForApi });
        } else if(item === 'review' && this.invalidFormsOnPassengerInfoPage){
